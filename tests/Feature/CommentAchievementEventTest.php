...
class CommentAchievementEventTest extends TestCase
{
    use DatabaseTransactions;
    use MakesHttpRequests;

    private CommentWrittenListener $listener;
    ...
    public function setUp(): void
    {
        ...
        $this->listener = app(CommentWrittenListener::class);
    }
    ...
    public function testThreeCommentsAchievement()
    {
        ...
        for ($i = 0; $i < 3; $i++) {
            $this->dispatchCommentWrittenEvent(Comment::factory()->create(['user_id' => $user->id, 'body' => 'Third Comment']));
        }
    ...
    public function testFiveCommentsAchievement()
    {
        ...
        // Dispatch three CommentWritten events
        for ($i = 0; $i < 5; $i++) {
            $this->dispatchCommentWrittenEvent(Comment::factory()->create(['user_id' => $user->id, 'body' => 'Fifth Comment']));
        }
    ...
    public function testTenCommentsAchievement()
    {
        ...
        // Dispatch three CommentWritten events
        for ($i = 0; $i < 10; $i++) {
            $this->dispatchCommentWrittenEvent(Comment::factory()->create(['user_id' => $user->id, 'body' => 'Ten Comment']));
        }
    ...
    public function testTwentyCommentsAchievement()
    {
        ...
        for ($i = 0; $i < 20; $i++) {
            $this->dispatchCommentWrittenEvent(Comment::factory()->create(['user_id' => $user->id, 'body' => 'Twenty Comment']));
        }
    ...
    private function dispatchCommentWrittenEvent(Comment $comment): void
    {
        $event = new CommentWritten($comment);
        event($event);
